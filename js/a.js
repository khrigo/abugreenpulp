...
function t766_showPopup(recid){
  ...
  el.find('.t-popup').mousedown(function(e){
    if (e.target == this) { t766_closePopup(recid); }
  });
...
function t851_setHeight(rec) {
    var startHeight = rec.find('.t851__table').css('height');
    var col = rec.find('.t851__col');
    var ratio = rec.attr('data-tile-ratio');
    var ratioHeight = rec.find('.t851__col').width() * ratio;
    var blocksPerRow = rec.find('.t851__container').attr('data-blocks-per-row');
    var isOneRowOnMobile = rec.find('.t851__container').hasClass('t851__container_mobile-flex');

    if (typeof blocksPerRow != 'undefined') {
        blocksPerRow = +blocksPerRow;
        if ($(window).width() <= 960) {
            if (isOneRowOnMobile) {
                blocksPerRow = col.length;
            } else {
                blocksPerRow = 2;
            }
        }
        ...
        for (var i = 0; i < col.length; i += blocksPerRow) {
            var row = col.slice(i, i + blocksPerRow);
            var table = row.find('.t851__table');
            var largestHeight = 0;

            row.each(function() {
                var curText = $(this).find('.t851__textwrapper');
                var itemHeight = curText.outerHeight();
                if (itemHeight > largestHeight) {
                    largestHeight = itemHeight;
                }
            });

            if ($(window).width() > 480 || ($(window).width() <= 960 && isOneRowOnMobile)) {
                if (largestHeight > ratioHeight) {
                    table.css('height', largestHeight);
                } else {
                    table.css('height', ratioHeight);
                }
                table.css('min-height', 'auto');
            } else {
                table.css('min-height', startHeight);
                table.css('height', '');
            }
        }
    }
...
function t912_init(recid) {
    ...
    $(".t912").bind("displayChanged", function () {
        t912_setHeight(recid);
    });
